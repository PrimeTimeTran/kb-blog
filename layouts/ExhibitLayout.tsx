...
//   const { sizes, getHandleProps, isDragging } = useMultiSplitter(isPreview);
//   const { sidebarOpen } = useOverlayManager();
//   const className = `h-screen pt-16 flex flex-col overflow-hidden ${isPreview ? 'top-0' : 'top-0'}`;
//   return (
//     <div className={className}>
//       <div className={`flex flex-1 min-h-0 overflow-hidden ${isDragging ? 'select-none' : ''}`}>
//         <div
//           className="relative min-h-0 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] overflow-hidden"
//           style={{
//             width: sidebarOpen ? sizes[0] : 0,
...
//         <div style={{ width: sizes[1] }} className="relative flex-1 min-w-0 min-h-0">
//           <BaseScroll>{children || <SurfacePreviewMain />}</BaseScroll>
//         </div>
