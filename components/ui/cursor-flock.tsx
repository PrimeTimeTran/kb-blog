...
export function CursorFlock({ seed }: { seed?: string | null }) {
  ...
  useEffect(() => {
    ...
    const createParticle = (
      ...
      const randomColorHex = themeColors[Math.floor(Math.random() * themeColors.length)] || '#888888';
    ...
    const animate = () => {
      ...
      if (now > spawnState.current.nextToggleTime) {
        ...
        const duration = spawnState.current.active ? Math.random() * 2000 + 2000 : Math.random() * 4000 + 2000;
      ...
      if (now > leaveState.current.nextToggleTime) {
        ...
        const duration = leaveState.current.active ? Math.random() * 2000 + 1000 : Math.random() * 7000;
      ...
      for (let i = particles.current.length - 1; i >= 0; i--) {
        ...
        // Removal Check
        const isOffScreen = p.x < -100 || p.x > canvas.width + 100 || p.y < -100 || p.y > canvas.height + 100;
