...
export default function DesignKitPage() {
  ...
  useEffect(() => {
    ...
    const vars = generateTheme(currentSeed, resolvedTheme === 'dark' ? 'dark' : 'light')

    applyTheme(vars)
  }, [resolvedTheme, currentSeed])

  const currentPaletteName = PALETTES.find((p) => p.color === currentSeed)?.name || 'Default'

  if (!mounted) return null

  return (
    <div className="w-full h-full overflow-auto relative">
      <ThemeController currentSeed={currentSeed} onSeedChange={setCurrentSeed} />
      <SectionNavigation />
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-4">
              ...
              <div className="text-sm font-mono text-primary pt-2 h-6">
                <Typewriter
                  strings={[`Press ⌥ + T to toggle ☀️ /🌙 modes`, `Press ⌥ + C to cycle 🎨 themes`]}
                  typeSpeed={70}
                  deleteSpeed={50}
                  delayBetween={3000}
                />
              ...
              <p className="text-muted-foreground text-lg max-w-2xl">
                Comprehensive design system including tokens, colors, typography, and components
                used across web development.
              </p>
            </div>
          </div>
        </div>
